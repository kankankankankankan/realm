...
Update_Shell() {
    ...
    sh_new_ver=$(wget --no-check-certificate -qO- "https://github.palees.com/https://raw.githubusercontent.com/wcwq98/realm/main/realm.sh" | grep 'sh_ver="' | awk -F "=" '{print $NF}' | sed 's/\"//g' | head -1)
    ...
    yn=${yn:-y}
    if [[ ${yn} =~ ^[Yy]$ ]]; then
        wget -N --no-check-certificate https://github.palees.com/https://raw.githubusercontent.com/wcwq98/realm/main/realm.sh -O realm.sh
        if [[ $? -ne 0 ]]; then
...
deploy_realm() {
    ...
    _version=$(curl -s https://github.palees.com/https://api.github.com/repos/zhboner/realm/releases/latest | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
    ...
    case "$arch-$os" in
        x86_64-linux)
            download_url="https://github.palees.com/https://github.com/zhboner/realm/releases/download/${_version}/realm-x86_64-unknown-linux-gnu.tar.gz"
            ;;
        x86_64-darwin)
            download_url="https://github.palees.com/https://github.com/zhboner/realm/releases/download/${_version}/realm-x86_64-apple-darwin.tar.gz"
            ;;
        aarch64-linux)
            download_url="https://github.palees.com/https://github.com/zhboner/realm/releases/download/${_version}/realm-aarch64-unknown-linux-gnu.tar.gz"
            ;;
        aarch64-darwin)
            download_url="https://github.palees.com/https://github.com/zhboner/realm/releases/download/${_version}/realm-aarch64-apple-darwin.tar.gz"
            ;;
        arm-linux)
            download_url="https://github.palees.com/https://github.com/zhboner/realm/releases/download/${_version}/realm-arm-unknown-linux-gnueabi.tar.gz"
            ;;
        armv7-linux)
            download_url="https://github.palees.com/https://github.com/zhboner/realm/releases/download/${_version}/realm-armv7-unknown-linux-gnueabi.tar.gz"
            ;;
        *)
            echo "不支持的架构: $arch-$os"
            return
...
update_realm() {
    ...
    tag_version=$(curl -Ls "https://github.palees.com/https://api.github.com/repos/zhboner/realm/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
    ...
    arch=$(uname -m)
    wget -N --no-check-certificate -O /root/realm/realm.tar.gz "https://github.palees.com/https://github.com/zhboner/realm/releases/download/${tag_version}/realm-${arch}-unknown-linux-gnu.tar.gz"
...
install_panel() {
    ...
    # 下载面板文件
    download_url="https://github.palees.com/https://github.com/wcwq98/realm/releases/download/v2.1/${panel_file}"
